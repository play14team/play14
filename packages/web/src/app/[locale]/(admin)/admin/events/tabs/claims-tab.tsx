...
function MyClaims({ claims, onClaimCancelled }: MyClaimsProps) {
  ...
  const handleCancel = async (claimId: string) => {
    if (!confirm(t("claims.confirmCancel"))) return
    ...
    if (result.success) {
      ...
      setError(result.error || t("claims.failedToCancel"))
    }
          ...
          {pendingClaims.map((claim) => (
            <div key={claim.documentId} className="my-claim-item my-claim-item-pending">
              ...
              <div className="my-claim-item-content">
                <h4 className="my-claim-item-name">
                  {claim.event?.name || t("claims.unknownEvent")}
                </h4>
...
function EventSearch({ events, onSelectEvent }: EventSearchProps) {
  ...
  if (events.length === 0 && !searchQuery) {
    return (
      <EventsEmptyState
        ...
        title={t("claims.noEventsToClaim")}
        message={t("claims.noEventsToClaimMessage")}
        action={{
  ...
  return (
    <div className="events-list">
      <EventFilterBar
        searchEnabled
        searchPlaceholder={t("search.searchByNameOrLocation")}
        searchValue={searchQuery}
        onSearchChange={setSearchQuery}
        isSearching={isSearching}
        filteredCount={displayEvents.length}
        totalCount={events.length}
        countLabel={t("claims.eventsAvailableToClaim")}
      />
        ...
        <div className="events-grid">
          {displayEvents.map((event) => (
            <EventCard
              ...
              action={{
                type: "button",
                label: t("claims.claimAttendance"),
...
function ClaimForm({ event, onBack, onSubmitted }: ClaimFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (reason.length < 20) {
      setError(t("claims.reasonMinLength"))
      return
    }
    ...
    if (result.success) {
      ...
      setError(result.error || t("claims.failedToSubmit"))
    ...
    <div className="claim-form">
      ...
      <form onSubmit={handleSubmit} className="claim-form-fields">
        <div className="form-group">
          <label htmlFor="reason">{t("claims.reasonLabel")}</label>
          <textarea
            id="reason"
            value={reason}
          ...
          >
            {t("claims.charactersMinimum", { count: reason.length })}
        ...
        <div className="event-search-hint" style={{ marginTop: "8px" }}>
          ...
          <span>{t("claims.reviewNotice")}</span>
        </div>
      </form>
    </div>
  )
