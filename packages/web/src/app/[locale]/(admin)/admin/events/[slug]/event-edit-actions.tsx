    ...
    <div className="event-edit-actions">
      ...
      <div className="action-status">
        ...
        <p className="status-description">
          {isPublished ? t("editActions.publishedDescription") : t("editActions.draftDescription")}
        </p>
      </div>
      ...
      <div className="action-buttons">
        ...
        >
          {isPublishing ? (
            <>
              ...
              {isPublished ? t("editActions.unpublishing") : t("editActions.publishing")}
            </>
          ) : isPublished ? (
            <>
              <i className="bx bx-hide" />
              {t("editActions.unpublish")}
            </>
          ) : (
            <>
              <i className="bx bx-globe" />
              {t("editActions.publish")}
            </>
          )}
        </button>
        ...
        >
          ...
          {isRegistrationOpen
            ? t("editActions.registrationOpen")
            : t("editActions.openRegistration")}
        </button>
        ...
        >
          {isSubmitting ? (
            <>
              <i className="bx bx-loader-alt bx-spin" />
              {t("editActions.saving")}
            </>
          ) : (
            <>
              <i className="bx bx-save" />
              {t("editActions.saveChanges")}
            </>
          )}
        </button>
          ...
          <div className="action-budget-summary">
            ...
            {activeTab === "budget" ? (
              <div className="budget-summary-compact">
                ...
                >
                  <span className="summary-label">{t("editActions.margin")}</span>
              ...
              <div className="budget-summary-compact">
                <div className="summary-section-label">{t("editActions.projected")}</div>
                <div
                  className={`summary-row margin ${budgetSummary.margin >= 0 ? "positive" : "negative"}`}
                >
                  <span className="summary-label">{t("editActions.projected")}</span>
                  <span className="summary-value">
                    {budgetSummary.margin >= 0 ? "+" : ""}
                    {fmt(budgetSummary.margin)}
                  </span>
                </div>
                <div className="summary-section-label">{t("editActions.actuals")}</div>
                <div className="summary-row income">
                  <span className="summary-label">{t("editActions.income")}</span>
                ...
                >
                  <span className="summary-label">{t("editActions.result")}</span>
                  <span className="summary-value">
      ...
      {isDirty && (
        <div className="dirty-indicator">
          ...
          <span>{t("editActions.unsavedChanges")}</span>
        </div>
      )}
    </div>
  )
