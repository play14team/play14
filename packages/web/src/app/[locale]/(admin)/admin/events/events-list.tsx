  ...
  if (events.length === 0) {
    return (
      <div className="claims-empty">
        ...
        <h3>{t("list.noEventsYet")}</h3>
        <p>{t("list.noEventsCreated")}</p>
        <Link href="/admin/events/create" className="admin-btn admin-btn-primary">
          <i className="bx bx-plus" />
          {t("list.createFirstEvent")}
        </Link>
      </div>
    )
  }
    ...
    <div className="events-list">
      <div className="events-toolbar">
        ...
        <div className="events-count">
          {statusFilter !== "all"
            ? t("list.eventsCountTotal", { count: filteredEvents.length, total: events.length })
            : t("list.eventsCount", { count: filteredEvents.length })}
        </div>
      </div>
      ...
      {filteredEvents.length === 0 ? (
        <div className="events-empty-filter">
          ...
          <p>
            {t("list.noFilteredEvents", {
              filter: statusFilter === "active" ? t("filters.active").toLowerCase() : statusFilter,
            })}
          </p>
          {statusFilter !== "all" && (
            <button
              ...
              <Link href={`/admin/events/${event.slug}`} className="event-card">
                <div className="event-card-header">
                  ...
                  <div className="event-card-badges">
                    {!event.isPublished && (
                      <span className="event-badge event-badge-draft">{t("list.draft")}</span>
                    )}
                    <span
