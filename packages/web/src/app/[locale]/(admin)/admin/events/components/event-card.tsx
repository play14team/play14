...
function getSourceIcon(source: "ticket" | "claim" | "direct"): string {
  switch (source) {
    case "ticket":
...
function getSourceLabelKey(source: "ticket" | "claim" | "direct"): string {
  switch (source) {
    case "ticket":
      return "card.ticket"
    case "claim":
...
function Badge({ badge }: { badge: BadgeType }) {
  ...
  switch (badge.type) {
    ...
    case "draft":
      return <span className="event-card-badge event-card-badge-draft">{t("card.draft")}</span>
    case "source": {
      const icon = getSourceIcon(badge.value)
      const label = t(getSourceLabelKey(badge.value))
      return (
        <span
