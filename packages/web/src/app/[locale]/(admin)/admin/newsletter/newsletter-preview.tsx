...
export default function NewsletterPreview({
  ...
  const t = useTranslations("adminMisc.newsletter.preview")
  ...
  useEffect(() => {
    async function loadPreview() {
      ...
      if (!saveResult.success) {
        setError(saveResult.error || t("saveFailed"))
        setIsLoading(false)
        return
      }
        ...
        <div className="modal-body newsletter-preview-body">
          ...
          {previewHtml && !isLoading && (
            <div className={`newsletter-preview-frame newsletter-preview-${device}`}>
              <iframe srcDoc={previewHtml} title={t("title")} sandbox="allow-same-origin" />
            </div>
          )}
        </div>
