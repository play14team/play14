...
const RunInfo = ({ run, label }: { run?: WorkflowRun | null; label: string }) => {
  if (!run) return null

  return (
    <Box padding={3} background="neutral100" borderColor="neutral200" hasRadius>
...
const RebuildWidget = () => {
  ...
  const [latestSuccessfulRun, setLatestSuccessfulRun] = useState<WorkflowRun | null>(null)
  const [currentRun, setCurrentRun] = useState<WorkflowRun | null>(null)
  const [isPolling, setIsPolling] = useState(false)
  const triggeredRunId = useRef<number | null>(null)
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      const { latestRun, latestSuccessfulRun: successfulRun } =
        (response?.data as { latestRun?: WorkflowRun; latestSuccessfulRun?: WorkflowRun }) || {}

      setLatestSuccessfulRun(successfulRun || null)
  ...
  const handleRebuild = async () => {
    ...
    try {
      ...
      toggleNotification({
        type: "warning",
        message: getErrorMessage(error, "Failed to trigger rebuild"),
      })
    } finally {
      setLoading(false)
    }
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      toggleNotification({
        type: "warning",
        message: getErrorMessage(error, "Failed to cancel workflow run"),
      })
    } finally {
      setCancelling(false)
    }
  ...
  const isBuilding =
    currentRun && (currentRun.status === "in_progress" || currentRun.status === "queued")
  const hasFailed =
    currentRun && currentRun.status === "completed" && currentRun.conclusion === "failure"
