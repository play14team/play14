...
export async function up(knex) {
  ...
  await knex.transaction(async (trx) => {
    const hasTicketOrdersTable = await trx.schema.hasTable("ticket_orders")
    ...
    const hasStatusColumn = await trx.schema.hasColumn("ticket_orders", "status")
    const hasOrderStatusColumn = await trx.schema.hasColumn("ticket_orders", "order_status")

    if (hasStatusColumn && !hasOrderStatusColumn) {
      console.log("Renaming status column to order_status in ticket_orders table")

      await trx.schema.alterTable("ticket_orders", (table) => {
