...
export async function up(knex) {
  ...
  const playerRole = await knex("up_roles").select("id").where({ type: "player" }).first()
  ...
  for (const roleColumn of roleColumnCandidates) {
    ...
    try {
      ...
      console.log(`Updated ${updated} user(s) to player role via up_users.${roleColumn}`)
      return
    } catch (error) {
...
export async function down() {
  console.log("Rollback skipped: role migration is not safely reversible without audit data")
}
